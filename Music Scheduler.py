...
class SpotifyApp:
    ...
    def schedule_song_playback(self, day, time_str, url, loop):
        # If there's an existing scheduled thread for the day, stop it
        if day in self.scheduled_threads:
            self.scheduled_threads[day]['event'].set()

        stop_event = threading.Event()
        self.scheduled_threads[day] = {
            'thread': threading.Thread(target=self.play_at_time, args=(day, time_str, url, loop, stop_event), daemon=True),
            'event': stop_event
        }
        self.scheduled_threads[day]['thread'].start()
    ...
    def save_urls(self):
        ...
        for day, data in self.urls.items():
            ...
            if url:
                ...
                self.schedule_song_playback(day, data['time'].get(), url, data['loop'].get())
                self.schedule_song_playback(day, data['lunch_time'].get(), url, data['loop'].get())
